...
function Get-VMWareRAVMID {
    ...
    Process
    {
        ...
        if(($vmID = (Get-VMWareRAVM -vCenter $vCenter -sessionID $sessionID -computer $computer).id))
        {
            return $vmID
        }
        else{return $null}
...
function Get-VMWareRATag {
    ...
    Process
    {
       ...
       if ($category)
        ...
        if($tagList.Count -eq 0){throw "Unable to find any tags that match criteria"}
        else{return $tagList}
